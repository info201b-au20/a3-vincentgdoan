...
trend_chart_data <- incarceration %>%
  ...
  mutate(
    proportion_black = total_black_pop / total_jail_pop,
...
trend_chart_data_part_two <- incarceration %>%
  ...
  mutate(
    proportion_black = total_black_pop / total_pop,
...
variable_comparison <- incarceration %>%
  group_by(year) %>%
  summarize(
    prison_admission_rate = sum(black_prison_adm_rate, na.rm = TRUE),
...
variable_comparison_scatterplot <- ggplot(
  data = variable_comparison) +
  geom_point(mapping = aes(
    x = prison_admission_rate,
    y = prison_pop_rate
  )) +
  ggtitle(
    "Association of Prison Admission Rates and Prison Population Rates for Blacks"
  ) +
  xlab("Admission Rate") +
